...
@csrf 
    ...
    <div class="form-group col-sm-4"> 
        ...
        @endforeach @foreach($retur as $rtr) 
        ...
        <div class="card-body"> 
            ...
            <table class="table table-bordered table-striped" id="dataTable" width="100%" cellspacing="0"> 
                ...
                @foreach($beli as $bli) 
                    <tr> 
                        <td>
                            <input name="kd_barang[]" class="form-control" type="hidden" value="{{$bli->kd_barang}}" readonly>{{$bli->kd_barang}}
                        </td> 
                        <td>{{$bli->nm_barang}}</td> 
                        <td align="center"> 
                            <input name="qty_beli[]" class="form-control" type="hidden" value="{{$bli->qty_beli}}" readonly> 
                            <input name="harga[]" class="form-control" type="hidden" value="{{$bli->harga}}" readonly> {{$bli->qty_beli}}
                        </td> 
                        <td width=10%>
                            <input name="jml_retur[]" class="form-control" type="number" value="0">
                        </td> 
                        <td align="center"> 
                            <a href="/transaksi/hapus/{{$bli->kd_barang}}" onclick="return confirm('Yakin Ingin menghapus data?')" class="d-none d-sm-inline-block btn btn-sm btn-danger shadow-sm"> <i class="fas fa-trash-alt fa-sm text-white-50"></i> Hapus</a>
